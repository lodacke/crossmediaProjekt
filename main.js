...
export async function renderScoreBoard(user) {
    ...
    try {
        ...
        top3Users.forEach((user, index) => {
            ...
            if (user.games && user.games.length > 0) {
                ...
                let maxPoints = Math.min(...user.games.map(game => game.points));

                index++;
                let content = ``;
                if (index == 1) {
                    dom.classList.add("bounceWinner")
